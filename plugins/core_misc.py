...
def onjoin(conn, bot):
    ...
    #oper login
    #SLNL extension
    oper = conn.config.get('oper')
    if oper and oper.get("enabled", True):
        bot.logger.info("[{}|misc] Bot is getting his O-line.".format(conn.name))
        oper_pass = oper.get('oper_pass', '')
        oper_user = oper.get('oper_user', '')
        conn.cmd("OPER {} {}".format(oper_user, oper_pass))
        conn.part('#opers')
